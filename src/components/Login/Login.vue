<template>
  <div class="d-flex flex-column text-center justify-content-center align-items-center mt-4">
    ...
    <div v-if="!isInternalLogin">
      <mdl-button raised class="btn-facebook btn-block mdl-js-ripple-effect" @click.native.prevent="facebookLogin()">Log In with <i class="fa fa-facebook-official" title="Facebook"></i><span class="sr-only">Facebook</span></mdl-button>
      <p class="py-1 my-1">or</p>
      <mdl-button @click.native.prevent="swapLoginType()" colored raised class="btn-block mdl-js-ripple-effect">Log In</mdl-button>
      <div class="pt-1">
    ...
    <form class="d-flex flex-column align-items-center justify-content-center" v-if="isInternalLogin">
      ...
      <mdl-button accent raised class="mdl-js-ripple-effect" :disabled="isDisabled" @click.native.prevent="tryLogin()">Submit</mdl-button>
      <mdl-button class="mdl-js-ripple-effect" @click.native.prevent="swapLoginType()">Back</mdl-button>
    </form>
  </div>
</template>
